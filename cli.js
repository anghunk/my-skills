...
function showHelp() {
	console.log(`
askills - 管理 Claude Code Skills

用法:
  askills <命令> [选项]

命令:
  askills ls / list              列出所有可用 skills
  askills install <name>          安装 skills 到当前项目 (.claude/skills/)
  askills install <name> -g       全局安装 (~/.claude/skills/)
  askills uninstall <name>        卸载指定 skills
...
function installCommand(skillName, global) {
	...
	if (!skill) {
		console.error(`错误：找不到 skills "${skillName}"`);
		console.error('\n使用 "askills list" 查看可用 skills 列表。');
		process.exit(1);
	}
	...
	const doInstall = () => {
		try {
			...
			console.log(`✅ skills "${skillName}" 已安装到 ${targetPath}`);
		} catch (error) {
	...
	if (existsSync(targetPath)) {
		...
		rl.question(`skills "${skillName}" 已存在于 ${targetPath}\n是否覆盖？(yes/no): `, (answer) => {
			rl.close();
...
function uninstallCommand(skillName, global) {
	...
	if (!existsSync(targetPath)) {
		console.error(`❌ 错误：shills "${skillName}" 未安装`);
		return;
	}

	try {
		rmSync(targetPath, { recursive: true, force: true });
		console.log(`✅ skills "${skillName}" 已卸载`);
	} catch (error) {
	...
	switch (command) {
		...
		case 'install':
			if (!args[1] || args[1].startsWith('-')) {
				console.error('错误：请指定要安装的 skills 名称');
		...
		case 'uninstall':
			if (!args[1] || args[1].startsWith('-')) {
				console.error('错误：请指定要卸载的 skills 名称');
